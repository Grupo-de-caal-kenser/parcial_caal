...
const getEstadisticas = async () => {
    const url = `/parcial_caal/API/dato/estadistica`; 
    const config = {
        method: 'GET'
    };
    ...
    try {
        ...
        if (data) {
            ...
            const pendientesCount = data.filter(registro => registro.estado === 'Pendiente').length;
            chartEstados.data.datasets[0].backgroundColor.push(getRandomColor())
            updateChart([activosCount, inactivosCount, pendientesCount]);
        } else {
            Toast.fire({
